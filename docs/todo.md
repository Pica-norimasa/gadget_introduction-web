...
表示する機能)は、任意のURLをサーバー側から取得するためSSRF対策が必須。
`localhost`・`127.0.0.1`・`192.168.x`・`169.254.169.254`(AWSメタデータ)
等のプライベート/予約IPへのアクセスは、DNS解決した先のIPを見て拒否している
(`isPrivateOrReservedIp`)。

これとは別に、**自分自身の本番ドメインへのアクセスも塞いでいる**
(`isOwnHostname`)。`draftly-web.dev`はプライベートIPではない普通の公開
ドメインなので、IPベースの判定だけでは通ってしまい、この機能を悪用して
サーバー自身の内部専用ページ等を外部から間接的に叩けてしまうリスクが
あるため。

- 判定は`SITE_URL`(=`AUTH_URL`環境変数)から動的に行っているので、
  **本番ドメインを`draftly-web.dev`から別のドメインに変更する場合、
  `AUTH_URL`を更新しさえすればこのブロックも自動的に追従する
  (コード変更は不要)。**
- App Runnerの素のドメイン(`*.awsapprunner.com`、カスタムドメインとは
  別に常に生きている)は接尾辞マッチで別途ブロック済み。ホスティングを
  App Runner以外に移す場合は、この部分の関連性が無くなるので見直してよい。
- つまり「ドメイン再取得時に手で追加し忘れる」心配は無い設計にしてある。
  もし将来IPベースのSSRF対策自体を見直すなら、上記の考え方
  (プライベートIP拒否 + 自分自身のホスト名拒否)をセットで維持すること。
